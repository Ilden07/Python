...
def parse_polynomial(eq, real=False):
    ...
    try:
        ...
        print(f'Вводите только  {"real" if real else "natural"} числа в качестве коэффициентов.')
        return None
    ...
    try:
        ...
        print('Вводите только натуральные числа в качестве показателей.')
        return None
...
def main():
    polynomial1 = input('Введите первый многочлен: ')
    polynomial2 = input('Введите второй многочлен: ')
    parsed_polynomial = parse_polynomial(polynomial1 + '+' + polynomial2)

    print('Сумма двух многочленов:', print_polynomial(parsed_polynomial), sep='\n')
